...
def create_niimprint_agent(
    ...
    # Create agent with print and connection check tools
    agent = RequirementAgent(
        name="NiimprintAgent",
        llm=ChatModel.from_name(model, stream=True),
        tools=[CheckConnectionTool(), PrintImageTool()],
        memory=UnconstrainedMemory(),
        role="You are a helpful assistant that prints labels.",
        instructions=[
            "You have a B1 printer at /dev/cu.B1-H917122400.",
            ...
            "If print fails with 'NoneType' error, tell user:",
            "  'Printer connection needs warm-up. Try these steps:",
            "   1. Press the feed button on printer once (feeds blank paper)",
            "   2. Wait 5 seconds",
            "   3. Try printing again'",
            "Include the full error details in your response so user can see what went wrong.",
        ],
    )
